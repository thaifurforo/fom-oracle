  ...
  return (
    <div className="space-y-6">
      <section className="grid gap-4 xl:grid-cols-[1.35fr_0.9fr]">
        <Card className="border-sky-400/20 bg-gradient-to-br from-sky-500/10 via-white/5 to-transparent">
        ...
        <Card className="border-white/10 bg-white/5">
          ...
          <div className="mt-5 space-y-3">
            {nextSteps.map((step, index) => (
              <Card
                key={step}
                className="flex items-center gap-3 rounded-2xl border-white/10 bg-slate-950/50 px-4 py-3 shadow-none"
              >
                <span className="flex h-7 w-7 items-center justify-center rounded-full bg-sky-400/20 text-sm font-semibold text-sky-100">
                  {index + 1}
                </span>
