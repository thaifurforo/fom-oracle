...
| Validação de CSP só em build web pode ocultar divergências no webview Tauri empacotado | Segurança/Runtime | Média | PR #55 / thread PRRT_kwDOSZGoZs6CqVNl / reviewer copilot-pull-request-reviewer | A task T-02 cobre shell inicial e integração local; adicionar validação de pacote Tauri neste PR amplia escopo operacional além do ativo. | Reavaliar quando existir task específica para validação de `tauri build` em pipeline/QA do desktop. | 2026-05-17 |
| Drift entre aliases de Vite e Vitest pode causar inconsistência de resolução | Testes/Configuração | Média | PR #55 / thread PRRT_kwDOSZGoZs6CqdEW / reviewer copilot-pull-request-reviewer | A T-02 prioriza entrega funcional do shell; refatorar composição de configs Vite/Vitest amplia escopo para melhoria estrutural não obrigatória desta task. | Reavaliar em task de manutenção de tooling para centralizar aliases entre Vite e Vitest. | 2026-05-17 |
| Compatibilidade de `productName` e `identifier` com pacote NSIS não está validada em pipeline | Runtime/Empacotamento | Média | PR #55 / thread PRRT_kwDOSZGoZs6CqdEm / reviewer copilot-pull-request-reviewer | A T-02 não cobre hardening de empacotamento desktop; ampliar agora para validação NSIS extrapola escopo da entrega inicial do shell. | Reavaliar em task dedicada de release desktop com validação de `tauri build` e instalador NSIS. | 2026-05-17 |
| Gate de audit pode bloquear PRs por advisories high sem política formal de triagem | CI/Processo | Média | PR #55 / thread PRRT_kwDOSZGoZs6CqdE8 / reviewer copilot-pull-request-reviewer | Definir governança de bloqueio/non-blocking do audit é decisão de processo CI além do escopo funcional da T-02. | Reavaliar ao formalizar política de segurança de dependências e fluxo de exceções na CI. | 2026-05-17 |
| Prop `disableAutoRefetch` exposta como válvula de escape de testes na API pública de `ConnectionBanner` | Testes/API de Componente | Baixa | PR #55 / review harness-code-review / reviewer copilot | A T-02 priorizou entrega funcional do shell; refatorar o contrato de testes do componente amplia escopo sem ganho funcional imediato. | Substituir pela abordagem `vi.useFakeTimers()` ou configuração de `QueryClient` de teste sem `refetchInterval`, removendo a prop da interface pública do componente. | 2026-05-17 |
