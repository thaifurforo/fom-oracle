...
| Telas implementadas sem validação visual prévia podem divergir do `DESIGN.md` | UX/Governança | Média | T-39 | Mudanças visuais podem se afastar do guia se agentes pularem a etapa de protótipo e revisão manual prevista no fluxo de frontend/UI. | Orientar agentes a gerar protótipo usando preferencialmente Google Stitch MCP, revisar contra o `DESIGN.md` e manter gate automatizado apenas para regras objetivas do guia. | 2026-05-17 |
...
| Mensagem de panic em `main.rs` não inclui detalhes de diagnosticabilidade | Runtime/Desktop | Média | PR #55 / thread PRRT_kwDOSZGoZs6Crf5x / reviewer copilot-pull-request-reviewer | Fora do escopo T-02 (shell inicial mínimo); requer tratamento de erro explícito do Rust nativo e logging adequado para desktop. O `expect()` aborta silenciosamente. | Reavaliar em task de hardening de runtime desktop (T-05 ou posterior); substituir `expect()` por `if let Err(e)` com `eprintln!()` e saída clara para o usuário. | 2026-05-17 |
| Documentação/merge de CSP entre `tauri.conf.json` e `tauri.dev.conf.json` não está explícita | Documentação/Segurança | Baixa | PR #55 / thread PRRT_kwDOSZGoZs6Crf54 / reviewer copilot-pull-request-reviewer | Fora do escopo T-02 (shell UI inicial); requer task específica de hardening de CSP e documentação do mecanismo de merge do Tauri 2. As strings CSP divergem (`connect-src`: `ws://*:5173` em dev apenas). | Reavaliar em task de consolidação de CSP/segurança (T-06 ou posterior); extrair CSP para constante de build, documentar divergências intencionais e garantir sincronização. | 2026-05-17 |
| Validação robusta de `VITE_FOM_ORACLE_API_BASE_URL` ausente | Configuração/API | Média | PR #55 / thread PRRT_kwDOSZGoZs6Crj34 / reviewer copilot-pull-request-reviewer | Fora do escopo ativo da T-02, que cobre cliente HTTP placeholder e integração mínima com sidecar local; hardening completo de parsing/erro de configuração amplia escopo da task. | Reavaliar em task dedicada de hardening de configuração para validar esquema/host e emitir erro explícito de ambiente inválido. | 2026-05-17 |
| Granularidade de ignore do ESLint em `src-tauri/**` pode ocultar JS/TS futuro | Qualidade/Tooling | Baixa | PR #55 / thread PRRT_kwDOSZGoZs6Crj4K / reviewer copilot-pull-request-reviewer | Fora do escopo da T-02; a entrega atual não adiciona JS/TS em `src-tauri` e o ponto é preventivo para evolução futura. | Reavaliar em task de manutenção de lint para restringir ignore a artefatos de build quando houver scripts JS/TS em `src-tauri`. | 2026-05-17 |
| Piso mínimo da janela Tauri pode limitar uso em telas menores | UX/Desktop | Média | PR #55 / thread PRRT_kwDOSZGoZs6Crj4W / reviewer copilot-pull-request-reviewer | Fora do escopo ativo da T-02 e já direcionado para discussão de layout posterior (#58), sem expansão de responsividade nesta entrega. | Reavaliar junto da definição de layout responsivo no item #58 para ajustar `minWidth/minHeight` com critérios de usabilidade. | 2026-05-17 |
